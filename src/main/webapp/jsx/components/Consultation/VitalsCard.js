...
function VitalsCard({props}) {
    ...
    const loadOtherVisitsVitals = useCallback(async () => {
        ...
        try {
            ...
            if (response.data.length > 0) {
                ...
                const otherVisits = response.data.filter(vital => vital.visitId !== patientObj.visitId);
    ...
    }, [patientObj.id, patientObj.visitId, getLatestVitals, loadOtherVisitsVitals, loadPreviousConsultation]);
    ...
    const isAnyLoading = loadingVitals || loadingOtherVitals || loadingConsultations;
                            ...
                            >
                                <Label as='a' color='blue' style={{width:'100%'}}>
                                    <Typography className={classes.heading}>
                                        Current Vitals - {moment(latestVitals.captureDate).format("DD/MM/YYYY hh:mm A")}
                                    </Typography>
                                </Label>
                            </AccordionSummary>
                            <AccordionDetails style={{padding:'8px'}}>
                                <List celled style={{width:'100%'}}>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px',borderTop:'1px solid #fff', marginTop:'-5px' }}>
                                        Pulse <span style={{color:'rgb(153, 46, 98)'}} className="float-end"><b>{latestVitals.pulse} bpm</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        Respiratory Rate <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{latestVitals.respiratoryRate} bpm</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        Temperature <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{latestVitals.temperature} °C</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        Blood Pressure <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{latestVitals.systolic}/{latestVitals.diastolic}</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        Height <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{latestVitals.height} cm</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        Weight <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{latestVitals.bodyWeight} kg</b></span>
                                    </List.Item>
                                    <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                        BMI <span className="float-end"><b style={{color:'rgb(153, 46, 98)'}}>{(latestVitals.bodyWeight / Math.pow(latestVitals.height / 100, 2)).toFixed(1)}</b></span>
                                    </List.Item>
                                </List>
                            </AccordionDetails>
                        </Accordion>
                    ...
                    {loadingOtherVitals ? (
                        ...
                        otherVisitsVitals.map((vital, index) => (
                            <Accordion key={index}>
                                ...
                                >
                                    <Typography className={classes.heading} style={{color:'#014d88'}}>
                                        Previous Vitals - {moment(vital.captureDate).format("DD/MM/YYYY hh:mm A")}
                                    </Typography>
                                </AccordionSummary>
                                <AccordionDetails style={{padding:'8px'}}>
                                    <List celled style={{width:'100%'}}>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px',borderTop:'1px solid #fff', marginTop:'-5px' }}>
                                            Pulse <span style={{color:'#014d88'}} className="float-end"><b>{vital.pulse} bpm</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            Respiratory Rate <span className="float-end" style={{color:'#014d88'}}><b>{vital.respiratoryRate} bpm</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            Temperature <span className="float-end" style={{color:'#014d88'}}><b>{vital.temperature} °C</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            Blood Pressure <span className="float-end" style={{color:'#014d88'}}><b>{vital.systolic}/{vital.diastolic}</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            Height <span className="float-end" style={{color:'#014d88'}}><b>{vital.height} cm</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            Weight <span className="float-end" style={{color:'#014d88'}}><b>{vital.bodyWeight} kg</b></span>
                                        </List.Item>
                                        <List.Item style={{paddingBottom:'10px', paddingTop:'10px'}}>
                                            BMI <span className="float-end" style={{color:'#014d88'}}><b>{(vital.bodyWeight / Math.pow(vital.height / 100, 2)).toFixed(1)}</b></span>
                                        </List.Item>
                                    </List>
                                </AccordionDetails>
                            </Accordion>
                        ...
                        <List.Item>
                            ...
                            >
                                <Button 
                                    icon 
                                    labelPosition='right' 
                                    style={{width:'100%',backgroundColor:'#992E62',color:"#fff", padding:'15px'}} 
                                    fluid
                    ...
                    {loadingConsultations ? (
                        ...
                        <Card style={{width:'100%'}}>
                            <Card.Content style={{padding:'5px'}}>
                                <Feed>
                                    {previousConsultation.map((consultation, index) => (
                                        <Accordion key={index}>
                                            <AccordionSummary
