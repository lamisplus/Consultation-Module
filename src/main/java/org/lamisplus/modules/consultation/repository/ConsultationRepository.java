...
public interface ConsultationRepository extends JpaRepository<Consultation, Integer> {
        ...
        @Query(value = "SELECT c.* FROM consultation c " +
                        "LEFT JOIN consultation_complaint pc ON c.id = pc.consultation_id " +
                        "LEFT JOIN consultation_diagnosis d ON c.id = d.consultation_id " +
                        "WHERE c.visit_id = :visitId", nativeQuery = true)
        ...
        @Query(value = "SELECT p.date_of_birth FROM patient_person p WHERE p.id = :patientId", nativeQuery = true)
        LocalDate findPatientDateOfBirthByPatientId(@Param("patientId") int patientId);
        ...
        @Query(value = "SELECT \n" +
                        ...
                        "\tLOWER(p.hospital_number) LIKE LOWER(CONCAT('%', :keyword, '%'))", nativeQuery = true)
        Page<PatientConsultationProjection> findByPatientIdWithConsultation(@Param("keyword") String keyword,
                        Pageable pageable);
}
